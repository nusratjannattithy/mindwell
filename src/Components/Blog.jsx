...
Why This Helps: Educating yourself about depression helps you better understand what your loved one is going through. It allows you to respond more appropriately and sensitively, making them feel understood rather than judged or misunderstood.
 ...
 `,
        ...
        {
            id: 5,
            title: "Positive Affirmations For Better Mental Health: A Brief Guide",
...
The benefits of affirmations can extend beyond the individual. When you adopt a positive mindset, it can influence your interactions with others, creating a ripple effect of positivity. By embodying the principles of your affirmations, you can inspire those around you. This can lead to improved relationships and a more positive atmosphere in your home, workplace, and community. Positive affirmations, therefore, have the potential not only to transform the individual but also to contribute to a more optimistic and supportive society.
        ...
        {
            id: 6,
            title: "5 Ways to Build Resilience and Emotional Intelligence in Students",
...
Facilitating a group discussion on healthy habits is an excellent way to get students thinking about their health and well-being. Make a list of your students’ suggestions and ask them to name one healthy habit, such as eating five pieces of fruit and veggies per day. `,
    ...
    return (
        <div className="w-full min-h-screen p-8 bg-blue-50">
            <h1 className="text-4xl font-bold text-[#1A0B5B] text-center mb-4">Mental Wellness Blog</h1>
            <p className="text-xl text-gray-700 text-center mb-6">Explore expert insights and practical tips for mental well-being.</p>
            ...
            <div className="grid md:grid-cols-3 gap-8">
                {filteredBlogs.map(blog => (
                    expanded === blog.id ? null : (
                        <div key={blog.id} className="bg-white shadow-lg rounded-lg overflow-hidden">
                            <img src={blog.image} alt={blog.title} className="w-full h-48 object-cover" />
                            <div className="p-4">
                                <h2 
                                    className="text-2xl font-bold text-[#1A0B5B] mb-2 cursor-pointer"
                                ...
                                <button
                                    onClick={() => toggleExpand(blog.id)}
                                    className="mt-4 px-6 py-3 text-white bg-[#1A0B5B] rounded hover:bg-[#120A3C] w-full"
                                >
                                    Explore
                                </button>
                            </div>
            ...
            {expanded && (
                <div className="w-full max-w-5xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden p-6 mt-8 md:col-span-3">
                    <div className="flex justify-center">
                        <img 
                            src={blogs.find(blog => blog.id === expanded).image} 
                            alt={blogs.find(blog => blog.id === expanded).title} 
                            className="w-auto h-[20rem] object-contain rounded-md"
                        />
                    </div>
                    <h2 className="text-3xl font-bold text-[#1A0B5B] mt-4">{blogs.find(blog => blog.id === expanded).title}</h2>
                    <p className="text-gray-700 mt-4 whitespace-pre-line">{blogs.find(blog => blog.id === expanded).fullContent || blogs.find(blog => blog.id === expanded).description}</p>
                    <button
            ...
            <div className="w-full flex flex-col items-center mt-12 bg-[#ffffff] p-8 rounded-lg shadow-lg">
                ...
                <p className="text-lg text-gray-700 mb-6 text-center max-w-lg">Dive deeper into knowledge with our collection of insightful articles and recommended books that enrich your mental wellness journey.</p>
